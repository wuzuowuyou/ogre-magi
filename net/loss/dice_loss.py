...
class AdaptiveDiceLoss(nn.Module):
    ...
    def forward(self, pred, batch):
        ...
        assert 'binary_k' in pred
        assert 'thresh_binary_k' in pred
        assert 'binary_v' in pred
        assert 'thresh_binary_v' in pred
        mask = batch['mask']

        binary_k = pred['binary_k']
        thresh_binary_k = pred['thresh_binary_k']
        gt_k = batch['gt_k']
        main_loss_k = self.main_loss(binary_k, gt_k, mask)
        thresh_loss_k = self.thresh_loss(thresh_binary_k, gt_k, mask)

        binary_v = pred['binary_v']
        thresh_binary_v = pred['thresh_binary_v']
        gt_v = batch['gt_v']
        main_loss_v = self.main_loss(binary_v, gt_v, mask)
        thresh_loss_v = self.thresh_loss(thresh_binary_v, gt_v, mask)

        main_loss = main_loss_k + main_loss_v
        thresh_loss = thresh_loss_k + thresh_loss_v
